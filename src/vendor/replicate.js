  ...
  constructor({
    ...
    this.models = {
      get: (path, version = null) =>
        ReplicateModel.fetch({ path, version, replicate: this }),
    };
  }
  async getModel(path) {
...
export class ReplicateModel {
  ...
  async *predictor(input) {
    ...
    console.log("starting response", startResponse);
...
export class DefaultFetchHTTPClient {
  ...
  async importFetch() {
    if (isNode && !globalThis.fetch) return; // globalThis.fetch = (await import("node-fetch"))["default"];
  }
  async get({ url }) {
