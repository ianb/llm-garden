...
function NameFixer({ model }) {
  const overlaps = model.domain.overlappingNames;
  console.log("overlaps...", overlaps);
  const keys = (overlaps && Object.keys(overlaps)) || [];
  keys.sort();
  return <div>
    <h3>Overlapping names</h3>
    <div>
      <Button onClick={() => model.domain.regenerateDuplicateNames()}>
        Refresh
      </Button>
    </div>
    {!overlaps ? <div>Not yet generated</div> : null}
    {overlaps && !keys.length ? <div>No overlapping names, cool!</div> : null}
    {overlaps && keys.length ? (
      <ol class="pl-2 border-l-2 nested-borders">
        {keys.map((name) => (
          <li class="bg-cyan-200 mb-4"><NameFixerConflict model={model} name={name} objectPaths={overlaps[name]} /></li>
        ))}
      </ol>)
      : null
    }
...
function NameFixerConflict({ model, name, objectPaths }) {
  const [renaming, setRenaming] = useState(null);
  const objects = [];
  const missing = [];
  for (const path of objectPaths) {
    const ob = model.domain.findObjectByPath(path);
    if (!ob) {
  ...
  function onRename(ob) {
    setRenaming(ob);
  }
  return <div>
    <div>
      Shared name: <span class="font-bold">{name}</span>
      <Button class="m-0 ml-4 py-0" onClick={() => model.domain.ignoreNameOverlap(name, objects)}>ignore</Button>
    </div>
    <ol class="pl-2 border-l-2 nested-borders">
      {objects.map((ob) => {
        const parent = model.domain.getParent(ob);
        return <li>
          <div>{ob.type} in <span class="font-bold">{parent && parent.name}</span></div>
          <Markdown text={model.domain.renderFieldDisplay(ob)} />
          {renaming === ob ?
            <NameFixerEdit model={model} object={ob} sharedName={name} onDone={() => setRenaming(null)} />
            : <Button class="py-0" onClick={() => onRename(ob)}>Rename</Button>}
        </li>;
      })}
      {missing.map((path) => <li>Missing: {path}</li>)}
    </ol>
  </div>;
}

function NameFixerEdit({ model, object, sharedName, onDone }) {
  const [nameOptions, setNameOptions] = useState(null);
  const [editing, setEditing] = useState(false);
  useEffect(() => {
    model.domain.generateAlternateNames(object, sharedName).then((names) => {
      setNameOptions(names.map(x => x.name));
    }).catch((err) => {
      setNameOptions([`Error: ${err}`]);
      console.error("Error generating alternate names", err);
    });
  }, []);
  if (!nameOptions) {
  ...
  async function onChoose(name) {
    setNameOptions(null);
    await model.domain.renameObject(object, name);
    onDone();
  }
  async function onRename(el) {
    const name = el.value.trim();
    if (name !== object.name) {
      setNameOptions(null);
      await model.domain.renameObject(object, el.value);
    }
    onDone();
  }
  async function onReroll() {
    setNameOptions(null);
    const names = await model.domain.generateAlternateNames(object, sharedName, true);
    setNameOptions(names.map(x => x.name));
  }
  console.log("render", nameOptions);
  return <ol class="ml-8 list-decimal">
    {nameOptions.map((name) => (
      <li
        class="cursor-default hover:bg-opacity-10 hover:bg-black"
        onClick={() => onChoose(name)}
      >{name}</li>
    ))}
    {editing ?
      <li><TextInput defaultValue={object.name} onSubmit={onRename} /></li>
      : <li class="cursor-default hover:bg-opacity-10 hover:bg-black" onClick={() => setEditing(true)}>Custom name</li>
    }
    <li class="cursor-default hover:bg-opacity-10 hover:bg-black" onClick={onReroll}>Reroll</li>
    <li class="cursor-default hover:bg-opacity-10 hover:bg-black" onClick={onDone}>Cancel</li>
  </ol>;
}
